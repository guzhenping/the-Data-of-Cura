...
class SceneView(openglGui.glGuiPanel):
	def __init__(self, parent):
		...
		group = []
		self.rotateToolButton = openglGui.glRadioButton(self, 8, _("Rotate"), (0,-1), group, self.OnToolSelect) # 旋转
		self.scaleToolButton  = openglGui.glRadioButton(self, 9, _("Scale"), (1,-1), group, self.OnToolSelect)    # 修改大小，坐标，规模
		self.mirrorToolButton  = openglGui.glRadioButton(self, 10, _("Mirror"), (2,-1), group, self.OnToolSelect)    # 最大化，重置 

		# 以下为测试代码，代码编号:20160305TEST001gzp，用于增加两个按钮	
		self.dageToolButton  = openglGui.glRadioButton(self, 11, _("Dage"), (3,-1), group, self.OnToolSelect)    # 大格 test one 
		self.dageTwoToolButton  = openglGui.glRadioButton(self, 12, _("Dage 2"), (4,-1), group, self.OnToolSelect)    # 大格 test two 

		self.resetRotationButton = openglGui.glButton(self, 12, _("Reset"), (0,-2), self.OnRotateReset)   # 三连排的按钮中，第一个的上面的Reset选项
		self.layFlatButton       = openglGui.glButton(self, 16, _("Lay flat"), (0,-3), self.OnLayFlat)    # 三连排的按钮中，第一个的上面的Lay flat选项

		self.resetScaleButton    = openglGui.glButton(self, 13, _("Reset"), (1,-2), self.OnScaleReset)    # 三连排的按钮中，第二个的上面的Reset
		self.scaleMaxButton      = openglGui.glButton(self, 17, _("To max"), (1,-3), self.OnScaleMax)    # 三连排的按钮中，第二个的上面的To Max，最大化

		self.mirrorXButton       = openglGui.glButton(self, 14, _("Mirror X"), (2,-2), lambda button: self.OnMirror(0))    # 旋转，改变视角
		self.mirrorYButton       = openglGui.glButton(self, 18, _("Mirror Y"), (2,-3), lambda button: self.OnMirror(1))    # 旋转，改变视角
		self.mirrorZButton       = openglGui.glButton(self, 22, _("Mirror Z"), (2,-4), lambda button: self.OnMirror(2))    # 旋转，改变视角
		...
		# Cura主界面中，右上角的图标View Mode
		# 传值给OnViewChange，调用了previewMode模块
	...
	def loadGCodeFile(self, filename):
		...
		#Cheat the engine results to loa d a GCode file into it.
		# 
	...
	def OnToolSelect(self, button):
		...
		# 以下为测试代码，代码编号:20160305TEST001gzp，用于增加两个按钮	
		elif self.dageToolButton.getSelected():
			self.tool = previewTools.toolNone(self)
		elif self.dageTwoToolButton.getSelected():
			self.tool = previewTools.toolNone(self)
		else:
			self.tool = previewTools.toolNone(self)
		...
		# 以下为测试代码，代码编号:20160305TEST001gzp，用于增加两个按钮	
		self.mirrorXButton.setHidden(not self.dageToolButton.getSelected())  # 本代码将影响原有功能，需要尽快删除。

		#self.mirrorYButton.setHidden(not self.dageToolButton.getSelected())
		#self.mirrorZButton.setHidden(not self.dageToolButton.getSelected())
        
        # 以下为测试代码，代码编号:20160305TEST001gzp，用于增加两个按钮	
        #self.mirrorYButton.setHidden(not self.dageTwoToolButton.getSelected())
		#self.dageTwoToolButton.setHidden(not self.dageTwoToolButton.getSelected())
		#self.dageTwoToolButton.setHidden(not self.dageTwoToolButton.getSelected())
	...
	# 实现右上角图标的功能，调用previewTools模块的功能
